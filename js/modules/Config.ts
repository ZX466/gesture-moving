/**
 * @fileoverview Central configuration for particle art system.
 */
...
export const AppConfig: AppConfigShape = {
    ...
    validateModel(modelId: unknown): string {
        return this.models.some(m => m.id === modelId) ? String(modelId) : 'heart';
    },

    createParticleConfig(overrides: ParticleConfigInput = {}): ParticleRuntimeConfig {
        return {
            defaultColor: this.particle.defaultColor,
            defaultCount: this.particle.defaultCount,
            minCount: this.particle.minCount,
            maxCount: this.particle.maxCount,
            defaultSize: this.particle.defaultSize,
            minSize: this.particle.minSize,
            maxSize: this.particle.maxSize,
            count: this.validate(overrides.count ?? this.particle.defaultCount, this.particle.minCount, this.particle.maxCount, this.particle.defaultCount),
            size: this.validate(overrides.size ?? this.particle.defaultSize, this.particle.minSize, this.particle.maxSize, this.particle.defaultSize),
            color: this.validateColor(overrides.color ?? this.particle.defaultColor),
            sensitivity: this.validate(overrides.sensitivity ?? this.gesture.defaultSensitivity, this.gesture.minSensitivity, this.gesture.maxSensitivity, this.gesture.defaultSensitivity),
        };
    },

    createGestureConfig(overrides: Partial<GestureConfig> = {}): GestureConfig {
        return { ...this.gesture, ...overrides };
    },

    createCameraConfig(overrides: Partial<CameraConfig> = {}): CameraConfig {
        return { ...this.camera, ...overrides };
    },
};
