/**
 * @fileoverview Shape generators for particle art system.
 * Each generator produces a Float32Array of [x, y, z] positions.
 */

import type { ShapeConfig } from '../types.js';
...
export const ShapeGenerators: Record<string, (count: number) => Float32Array> = {
    heart(count: number): Float32Array {
    ...
    flower(count: number): Float32Array {
        count = validateCount(count);
        const positions = new Float32Array(count * 3);
        const petals = 6;
        for (let i = 0; i < count; i++) {
    ...
    saturn(count: number): Float32Array {
        count = validateCount(count);
        const positions = new Float32Array(count * 3);
    ...
    buddha(count: number): Float32Array {
        count = validateCount(count);
        const positions = new Float32Array(count * 3);
        for (let i = 0; i < count; i++) {
            const idx = i * 3;
    ...
    fireworks(count: number): Float32Array {
        count = validateCount(count);
        const positions = new Float32Array(count * 3);
        const burstCount = 5;
    ...
    sphere(count: number): Float32Array {
        count = validateCount(count);
        const positions = new Float32Array(count * 3);
        for (let i = 0; i < count; i++) {
            const idx = i * 3;
    ...
    star(count: number): Float32Array {
        count = validateCount(count);
        const positions = new Float32Array(count * 3);
        const points = 5;
    ...
    spiral(count: number): Float32Array {
        count = validateCount(count);
        const positions = new Float32Array(count * 3);
        const turns = 4;
    ...
    galaxy(count: number): Float32Array {
        count = validateCount(count);
        const positions = new Float32Array(count * 3);
        const arms = 3;
        const armSpread = 0.4;
...
export type ShapeGeneratorKey = keyof typeof ShapeGenerators;
